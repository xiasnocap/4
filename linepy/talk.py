...
class Talk(object):
    ...
    def setcont(self,wait,sd,dd,ss,split,msg,tex,nama=[]):
        ...
        for a in range(k+1):
            ...
            for i in selection.parse()[a*20 : (a+1)*20]:
                ...
                if dd == 'delwl':
                    try:wait['whitelist'].remove(nama[i-1]);hh = 'Del WL'
    ...
    def getinformation(self,to,mid,data):
        try:
            if mid in data["whitelist"]:a = "Whitelisted: Yes"
    ...
    def datamentions(self, to, text, data, date, wait, ps=''):
        ...
        for aa in range(k+1):
            ...
            for i in data[aa*20 : (aa+1)*20]:
                no+=1
                if date == 'ADDWL':
                    if i in wait["whitelist"]:a = 'WL User'
                    else:
                        if i not in wait["blacklist"]:a = 'Add WL';wait["whitelist"].append(i)
                        else:a = 'BL User'
                if date == 'DELWL':
                    try:wait["whitelist"].remove(i);a = 'Del WL'
    ...
    def mentionmention(self, to, wait, text, dataMid=[], pl='', ps='', pg='', pt=[]):
        ...
        if pg == 'MENTIONALLUNSEND':self.unsendMessage(self.sendMessage(to, text, contentMetadata).id)
