...
def proCmd():
    key = '' if not settings['setKey']['status'] else settings['setKey']['key']
    with open('protection.txt', 'r') as f:
        text = f.read()
    proMessage = text.format(key=key.title())
    return proMessage
...
def groupCmd():
    ...
    with open('group.txt', 'r') as f:
        text = f.read()
    groupMessage = text.format(key=key.title())
    return groupMessage
...
def accessCmd():
    ...
    with open('access.txt', 'r') as f:
        text = f.read()
    accessMessage = text.format(key=key.title())
    return accessMessage

def optCmd():
    key = '' if not settings['setKey']['status'] else settings['setKey']['key']
    with open('option.txt', 'r') as f:
        text = f.read()
    optMessage = text.format(key=key.title())
    return optMessage
...
def setCmd():
    ...
    with open('settings.txt', 'r') as f:
        text = f.read()
    setMessage = text.format(key=key.title())
    return setMessage
        ...
        if op.type == 13 or op.type == 124:
            ...
            if op.param1 in status["promax"]:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    try:
                        d23X_19 = threading.Thread(target=cancel, args=(op.param1, op.param3)).start()
                        d23X_20 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                    except:
                        try:
                            inv1 = op.param3.replace('\x1e',',')
                            inv2 = inv1.split(',')
                            for _mid in inv2:
                                d23X_21 = threading.Thread(target=cancel, args=(op.param1, _mid)).start()
                                d23X_22 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        except:
                            try:
                                inv3 = op.param3.replace('\x1e',',')
                                inv4 = inv3.split(',')
                                for _mid in inv4:
                                    d23X_23 = threading.Thread(target=cancel, args=(op.param1, _mid)).start()
                                    d23X_24 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                            except:
                                pass
            if op.param2 in status["blacklist"]:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    try:
                        d23X_25 = threading.Thread(target=cancel, args=(op.param1, op.param3)).start()
                        d23X_26 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                    except:
                        try:
                            inv1 = op.param3.replace('\x1e',',')
                            inv2 = inv1.split(',')
                            for _mid in inv2:
                                d23X_27 = threading.Thread(target=cancel, args=(op.param1, _mid)).start()
                                d23X_28 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        except:
                            try:
                                inv3 = op.param3.replace('\x1e',',')
                                inv4 = inv3.split(',')
                                for _mid in inv4:
                                    d23X_29 = threading.Thread(target=cancel, args=(op.param1, _mid)).start()
                                    d23X_30 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                            except:
                                pass
            if op.param3 in status["blacklist"]:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    try:
                        d23X_31 = threading.Thread(target=cancel, args=(op.param1, op.param3)).start()
                        d23X_32 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                    except:
                        try:
                            inv1 = op.param3.replace('\x1e',',')
                            inv2 = inv1.split(',')
                            for _mid in inv2:
                                d23X_33 = threading.Thread(target=cancel, args=(op.param1, _mid)).start()
                                d23X_34 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        except:
                            try:
                                inv3 = op.param3.replace('\x1e',',')
                                inv4 = inv3.split(',')
                                for _mid in inv4:
                                    d23X_35 = threading.Thread(target=cancel, args=(op.param1, _mid)).start()
                                    d23X_36 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                            except:
                                pass
            if M001D23 in op.param3:
                if settings["autoJoin"] == True:
                    if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                        try:
                            d23X_37 = threading.Thread(target=join, args=(op.param1,)).start()
                        except:
                        	pass
                    else:
                        try:
                            d23X_38 = threading.Thread(target=reject, args=(op.param1,)).start()
                        except:
                            pass
            if M002D23 in op.param3:
                if settings["autoJoin"] == True:
                    if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                        try:
                            d23X_39 = threading.Thread(target=join, args=(op.param1,)).start()
                        except:
                        	pass
                    else:
                        try:
                            d23X_40 = threading.Thread(target=reject, args=(op.param1,)).start()
                        except:
                            pass
            ...
            if op.param1 in status["promax"]:
                if op.param2 in status["blacklist"]:
                    if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                        ...
                        try:
                            d23X_41 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        except:
                            pass
        if op.type == 19 or op.type == 133:
            ...
            if op.param1 in status["promax"]:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_42 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
                        d23X_43 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        d23X_44 = threading.Thread(target=invite, args=(op.param1, op.param3)).start()
                    except:
                        pass
            if op.param3 in M001D23:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_45 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
            ...
            if op.param3 in M002D23:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_50 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
            ...
            if op.param3 in M003D23:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_55 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
                        d23X_56 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        d23X_57 = threading.Thread(target=invite, args=(op.param1, op.param3)).start()
                        d23X_58 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                    except:
                        pass
        if op.type == 32 or op.type == 126:
            ...
            if op.param1 in status["promax"]:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_59 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
                        d23X_60 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        d23X_61 = threading.Thread(target=invite, args=(op.param1, op.param3)).start()
                    except:
                        pass
            if op.param3 == M001D23:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_62 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
                        d23X_63 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        d23X_64 = threading.Thread(target=backup, args=(op.param1, op.param3)).start()
                    except:
                        pass
            if op.param3 == M002D23:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_65 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
                        d23X_66 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        d23X_67 = threading.Thread(target=backup, args=(op.param1, op.param3)).start()
                    except:
                        pass
            if op.param3 == M003D23:
                if op.param2 in creator or op.param2 in owner or op.param2 in admin or op.param2 in staff or op.param2 in Bots or op.param2 in mybots:
                    ...
                    d23X_68 = threading.Thread(target=blacklist, args=(op.param2,)).start()
                    try:
                        d23X_69 = threading.Thread(target=kick, args=(op.param1, op.param2)).start()
                        d23X_70 = threading.Thread(target=invite, args=(op.param1, op.param3)).start()
                    except:
                        pass
                    ...
                    for hlth in hellterhead.split(' & '):
                        if hlth == "help":
                            if sender in creator or sender in owner or sender in admin or sender in staff:
                                sendFooter(receiver,helpCmd())
                        elif hlth == "protection":
                            if sender in creator or sender in owner or sender in admin or sender in staff:
                                sendFooter(receiver,proCmd())
                        elif hlth == "group":
                            if sender in creator or sender in owner or sender in admin or sender in staff:
                                sendFooter(receiver,groupCmd())
                        elif hlth == "access":
                            if sender in creator or sender in owner or sender in admin or sender in staff:
                                sendFooter(receiver,accessCmd())
                        elif hlth == "option":
                            if sender in creator or sender in owner or sender in admin or sender in staff:
                                sendFooter(receiver,optCmd())
                        elif hlth == "settings":
                            if sender in creator or sender in owner or sender in admin or sender in staff:
                                sendFooter(receiver,setCmd())
                            ...
                            dre += "██████╔╝██║░░██║███████╗██╗\n"
                            dre += "╚═════╝░╚═╝░░╚═╝╚══════╝╚═╝"
                            sendFooter(receiver,dre)
                        elif hlth == "me":
                            contact = a001.getContact(sender)
                            cover = a001.getProfileCoverURL(sender)
                            path = str(cover)
                            contentMetadata = {
                                'previewUrl': "http://dl.profile.line-cdn.net/" + contact.picturePath,
                                'i-installUrl': 'http://itunes.apple.com/app/linemusic/id966142320',
                                'type': 'mt',
                                'subText': contact.statusMessage if contact.statusMessage != '' else 'セルボットＤＲＥ！',
                                ...
                                'linkUri': 'https://music.me.me/launch?target=track&item=mb00000000016197ea&subitem=mt000000000d69e2db&cc=JP&from=lc&v=1',
                                'MSG_SENDER_ICON': "https://os.me.naver.jp/os/p/" + str(contact),
                                'MSG_SENDER_NAME':  contact.displayName,
                            }
                            a001.sendMessage(receiver, contact.displayName, contentMetadata, 19)
                        elif hlth == "mid":
                            contact = a001.getContact(sender)
                            ...
                            if sender in creator or sender in owner or sender in admin:
                                try:
                                    a001.inviteIntoGroup(receiver, [M002D23])
