...
def Anloki(msg,kamui,chatingan):
        if msg.text.lower() == 'announ clear':
            ...
            try:
                ...
                hlth = chatingan
                hlth.text = kamui.splittext(msg.text,'ss')
                hlth.link= 'line://nv/chatMsg?chatId={}&messageId={}'.format(msg.to,msg.id)
                hlth.displayFields = 1
                try:
                    hlth.thumbnail = "http://dl.profile.line-cdn.net/"+ kamui.getGroup(msg.to).pictureStatus
                except:
                    hlth.thumbnail = 'https://aditnugraha98.herokuapp.com/static/lang-logo.png'
                if msg.text.lower().startswith('announ create unlock '):kamui.createChatRoomAnnouncement(msg.to,0,hlth)
                if msg.text.lower().startswith('announ create all '):
                    a = kamui.getGroupIdsJoined()
                    for i in a:
                        try:
                            aas = kamui.talk.getRecentMessagesV2(i, 1)
                            hlth.link = 'line://nv/chatMsg?chatId={}&messageId={}'.format(i,aas[0].id)
                            G = kamui.getGroup(i).pictureStatus
                            hlth.thumbnail = "http://dl.profile.line-cdn.net/"+ G
                            kamui.createChatRoomAnnouncement(i,0,hlth)
                        except:
...
def Anlokiss(to,pesan,msg,chatingan):
        if pesan == 'announ clear':
            ...
            try:
                ...
                hlth = chatingan
                hlth.text = client.splittext(pesan,'ss')
                hlth.link= 'line://nv/chatMsg?chatId={}&messageId={}'.format(to,msg.id)
                hlth.displayFields = 1
                try:
                    hlth.thumbnail = "http://dl.profile.line-cdn.net/"+ kamui.getGroup(msg.to).pictureStatus
                except:
                    hlth.thumbnail = 'https://aditnugraha98.herokuapp.com/static/lang-logo.png'
                if pesan.startswith('announ create unlock '):kamui.createChatRoomAnnouncement(msg.to,0,hlth)
                if pesan.startswith('announ create all '):
                    ...
                    for i in a:
                        try:
                            ...
                            G = kamui.getGroup(i).pictureStatus
                            hlth.thumbnail = "http://dl.profile.line-cdn.net/"+ G
                            client.createChatRoomAnnouncement(i,0,hlth)
                        except:
                            e = traceback.format_exc()
                            open('w','w').write(str(e))
